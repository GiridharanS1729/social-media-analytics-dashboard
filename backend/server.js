...
async function fetchProfileData(username) {
  ...
  try {
    ...
    profileData.photo = await page.$eval('img[src*="profile_images"]', el => el.src).catch(() => '');
    profileData.name = await page.$eval('div[data-testid="UserName"] span', el => el.textContent).catch(() => 'Unknown User');
    profileData.posts = await page.$eval('.r-n6v787', el => el.textContent).catch(() => '');
    const [followingElement] = await page.$x(`//a[contains(@href, '/${username}/following')]//span`);
    profileData.following = followingElement ? await followingElement.evaluate(el => el.textContent) : '0';
    
    const [followersElement] = await page.$x(`//a[contains(@href, '/${username}/followers')]//span`);
    profileData.followers = followersElement ? await followersElement.evaluate(el => el.textContent) : '0';
    profileData.joined_date = await page.evaluate(() => {
      const spans = Array.from(document.querySelectorAll('span'));
      const joinedSpan = spans.find(span => span.textContent.includes('Joined'));
      return joinedSpan ? joinedSpan.textContent : 'Unknown';
    ...
    for (let i = 0; i < Math.min(posts.length, 5); i++) {
      const post = posts[i];
      const likes = await post.$eval('[data-testid="like"] span', el => el.textContent).catch(() => '0');
      const text = await post.$eval('div[data-testid="tweetText"]', el => el.textContent).catch(() => '');
      profileData.posts_data.push({ text, likes });
    }
